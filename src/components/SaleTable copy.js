...
const SaleTable = ({ results, counts, setSales, history }) => {
  ...
  const [url, setUrl] = useState(`${process.env.REACT_APP_API_URL}sales/`);
  ...
  const makeQuery = ({ name, value }) => {
    ...
    let url = `${process.env.REACT_APP_API_URL}sales/?`;
  ...
  const onChange = async (checkedValues) => {
    ...
    try {
      const { data } = await axios.get(
        // `${process.env.REACT_APP_API_URL}sales/?canceled=${qs}`
  ...
  const handleInputChange = async (e) => {
    ...
    try {
      const { data } = await axios.get(
        // `${process.env.REACT_APP_API_URL}sales/?customerId=${e.target.value}`
  ...
  const handleChange = async (e) => {
    ...
    try {
      const { data } = await axios.get(
        // `${process.env.REACT_APP_API_URL}sales/?page=${e.current}`
        makeQuery()
      );
      setSales(data);
    } catch (e) {
